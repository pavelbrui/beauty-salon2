...
export const BlogTeaser: React.FC = () => {
  ...
  return (
    <section className="py-20 bg-dark-100 border-y border-brand/10">
      <div className="max-w-6xl mx-auto px-4">
          ...
          {posts.map(p => {
            ...
            return (
              <LocalizedLink
                key={p.id}
                to={`/blog/${p.slug}`}
                className="group bg-dark-50 border border-brand/20 overflow-hidden transition-all duration-300 transform hover:-translate-y-1 hover:border-brand/40"
              >
                <div className="relative overflow-hidden h-48">
                  ...
                  <div className="absolute top-3 left-3">
                    <span className="px-2.5 py-1 bg-dark/90 backdrop-blur-sm text-brand rounded-full text-xs font-semibold">
                ...
                <div className="p-5">
                  <h3 className="font-display font-bold text-cream mb-2 line-clamp-2 group-hover:text-brand transition-colors">
                    {pTitle}
                  </h3>
                  {pExcerpt && (
                    <p className="text-cream-300 text-sm line-clamp-2 mb-3">{pExcerpt}</p>
                  )}
                  <div className="flex items-center justify-between text-xs text-cream-300">
        ...
        <div className="text-center">
          <LocalizedLink
            to="/blog"
            className="inline-flex items-center gap-2 px-6 py-3 bg-brand text-dark font-semibold hover:bg-brand-400 transition-colors"
          >
