...
export const UserBookings: React.FC = () => {
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      ...
      default:
        return { bg: 'bg-rose-100', text: 'text-rose-700', dot: 'bg-rose-500', label: t.profile_page?.statusPending || 'Oczekuje' };
    }
  };
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <h2 className="text-lg font-bold text-gray-900">
            ...
            {activeCount > 0 && (
              <span className="ml-2 inline-flex items-center justify-center px-2 py-0.5 text-xs font-medium bg-rose-100 text-rose-700 rounded-full">
                {activeCount}
              </span>
            )}
          </h2>
        ...
        {filteredBookings.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm p-12 text-center">
            <div className="mx-auto h-16 w-16 rounded-full bg-rose-50 flex items-center justify-center mb-4">
            ...
            <button
              ...
              className="px-5 py-2.5 bg-rose-500 text-white rounded-lg font-medium hover:bg-rose-600 transition-colors text-sm"
            >
                  ...
                  {calendarDays.map((day, i) => {
                    ...
                    return (
                      <div
                        ...
                        } ${isSelected ? 'bg-rose-50 ring-2 ring-rose-300 ring-inset' : ''} ${
                          dayBookings.length > 0 ? 'cursor-pointer hover:bg-rose-50/50' : ''
                        }`}
                      >
                        ...
                        {canQuickBook && (
                          <button
                            ...
                            className="absolute inset-0 m-auto w-fit h-fit flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-rose-500 text-white hover:bg-rose-600 hover:scale-105 transition-all shadow-md opacity-0 sm:group-hover:opacity-100 active:opacity-100 text-[11px] font-semibold whitespace-nowrap z-10"
