...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  return (
    ...
    >
      <div className="bg-dark-50 border border-brand/20 p-6 max-w-md w-full relative">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-cream-300 hover:text-cream transition-colors"
          aria-label="Close"
        >
        ...
        <p className="text-sm text-cream-300 mb-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-cream">{passwordLabel}</label>
            <input
              type="password"
              value={password}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-brand text-dark py-2.5 px-4 font-semibold hover:bg-brand-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
          >
            {loading ? (
              <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-dark" />
            ) : (
              mode === 'signin' ? t.auth.signIn : t.auth.signUp
            )}
          </button>
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-2 bg-dark-50 text-cream-300">{t.auth.orContinueWith}</span>
          </div>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="w-full flex items-center justify-center gap-3 border border-brand/30 py-2.5 px-4 hover:bg-brand/10 transition-colors font-medium text-cream disabled:opacity-50 disabled:cursor-not-allowed"
        >
        ...
        <div className="mt-4 text-center">
          <button
            ...
            className="text-sm text-brand hover:text-brand-400 transition-colors"
          >
