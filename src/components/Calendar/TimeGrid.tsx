      ...
      <div className="flex justify-center space-x-2">
        {(['morning', 'afternoon', 'evening'] as TimePeriod[]).map(period => (
          ...
          >
            {periodLabels[period]}
            {periodCounts[period] > 0 && (
              <span className={`ml-1.5 text-xs ${selectedPeriod === period ? 'text-brand-100' : 'text-gray-400'}`}>
                ({periodCounts[period]})
              </span>
            )}
          </button>
            ...
            {(['morning', 'afternoon', 'evening'] as TimePeriod[])
              ...
              .map(p => (
                <button
                  ...
                  className="mx-1 text-brand-600 hover:text-brand-600 underline"
                >
        ...
        return (
          <button
            ...
            className={`relative p-3 rounded-lg text-center transition-all duration-200 ${
              isSelected 
                ? 'bg-brand text-white ring-2 ring-brand-400 ring-offset-2 scale-105' 
                : !isSelected && available 
                  ? 'bg-brand-50 text-brand-600 hover:bg-brand-100 hover:scale-105 hover:shadow-md' 
                  : 'bg-gray-100 text-gray-400 cursor-not-allowed'
            }`}
          >
            {formatTime(slot.startTime)}
            {available && (
              <div className={`text-xs mt-1 ${isSelected ? 'text-white' : 'text-brand-600'}`}>
                {t.booking.available}
              </div>
            )}
            {isSelected && (
              <motion.div
                ...
                className="absolute -inset-px rounded-lg border-2 border-brand"
